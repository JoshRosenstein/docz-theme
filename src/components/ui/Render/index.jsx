...
const Actions = styled('div') `
  display: flex;
  padding: 0 5px;
  background: ${p => p.theme.docz.mode === 'light'
...
const Tab = styled('button') `
  ...
  border: none;
  font-size: 14px;
  color: ${(p) => rgba(textColor(p), p.active ? 0.8 : 0.4)};
  transition: color 0.3s;
`;
const storage = localStorage();
const get = (pos) => storage.get(pos.toString());
...
const Jsx = ({ children, ...props }) => (<Pre {...props} readOnly={false} className={editorClassName} actions={<Fragment />}>
    {children}
  </Pre>);
export class Render extends Component {
    ...
    get actions() {
        ...
        {codesandbox !== 'undefined' && (<ThemeConfig>
            {config => (<ActionLink href={this.codesandboxUrl(config.native)} target="_blank" title="Open in CodeSandbox">
    ...
    render() {
        ...
        return (<LiveProvider noInline code={this.state.code} scope={scope} transformCode={this.transformCode} mountStylesheet={false}>
          ...
          <Resizable {...this.resizableProps}>
            <Wrapper full={fullscreen}>
              <LiveConsumer key={key}>
                {(live) => (<PlaygroundWrapper full={fullscreen}>
                    {live.error && (<DummyPlayground className={className} style={style}>
              ...
              {this.actions}
              {showing === 'jsx' ? (<Jsx onChange={code => this.setState({ code })}>
                  {this.state.code}
                </Jsx>) : (<Pre className={editorClassName} actions={<Fragment />} withLastLine>
                  {this.html}
                </Pre>)}
            </Wrapper>
          </Resizable>
